...
class Client (object):
    ...
    api_read_url = "http://api.carriots.com/streams/IGUA_JOSE_V@igua.devs.igua.devs/"
...
##ser_flw =  serial.Serial('/dev/ttyACM0',9600,timeout = None)
...
#para carriots
device = "IGUA_JOSE_V@igua.devs.igua.devs"  # Replace with the id_developer of your device
...
apikey = "8971eb3a06dd2d55a7794f6c5c0067cbd8d349a04fd67fc611dc0dec552c41ce"
...
while 1 == 1:
	...
	if process_id == 0:  #espera monedas
		...
		if now != now_1:
			if modo_maquina == 0:
				...
				#MENSAJES DE INTRO
				#print(now)
				#lcd_bienvenida_linear(now)##LCD CAMBIAR##!!!!!
			if modo_maquina == 1:
				display_bienvenida_pwyw(now)
				#lcd_bienvenida_pwyw(now)  # cuidado CUIDADO!!!!##LCD CAMBIAR##!!!!!
    ...
    #leer aceptador de monedas
		...
		##bytesToRead = ser_acc.inWaiting()     OJOJOJOJOJOJ
                ##Lectura del acceptor
		try:
			number = bus.read_i2c_block_data(0x04,0,4)
			moneda = bus.read_byte(add_acc)
		except:
		...
		if number[0] > 0:
			sleep(0.5)
			#bytesToRead = ser_acc.inWaiting() OJOJOJOOJ
			#print("bytes to read: ", bytesToRead)
			#string_igua = str(ser_acc.readline(),'utf-8')
			#string_igua = ser_acc.read(2)	OJOJOJOOJ
			ferrosacumulados = 50
			solesacumulados = ferrosacumulados / 10.0
			before = int(time.time())
			if modo_maquina == 0:
				display_acumula_linear(solesacumulados)
				writeLCD(2,ferrosacumulados,0,0)
				#lcd_acumula_linear(solesacumulados)##LCD CAMBIAR##!!!!!
			if modo_maquina == 1:
				display_acumula_pwyw(solesacumulados)
				#lcd_acumula_pwyw(solesacumulados)##LCD CAMBIAR##!!!!!
	...
	#aceptando monedas
		...
		##bytesToRead = ser_acc.inWaiting() ojojojjoj
		print(moneda)
		if moneda > 0:
			sleep(0.5)
			#bytesToRead = ser_acc.inWaiting() OJOJOJOOJ
	...
	# habilitada vavula y muestra litros
		...
		# read_tds()
                ...
                #ser_flw.write('a'.encode()) FLUJOMETRO!!! listo
		#lectura_flujo = bus.read_i2c_block_data(0x07,0,2)
		#if lectura_flujo [1]!=255:
			#print(str(lectura_flujo )+" = "+str(lectura_flujo [0]+lectura_flujo [1]*128))
			#flujo=lectura_flujo [0]+lectura_flujo [1]*128


		#sleep(0.1)
		#read_flw() FLUJOMETRO!!! listo
		...
		while process_id == 3:
			...
			#ser_flw.write('a'.encode()) FLLUJOMETRO listo
			try:
				lectura_flujo = bus.read_i2c_block_data(0x06,0,2)
				if lectura_flujo [1]!=255:
 					flujo=lectura_flujo [0]+lectura_flujo [1]*128
 					print(str(lectura_flujo )+" = "+str(lectura_flujo [0]+lectura_flujo [1]*128))
				sleep(0.1)
			...
			#servidos_lt = float(int(string_flw)/10)*0.95 PARTE DE UNA FUNCION DEL FLUJOMETRO
			servidos_lt= float(int(flujo)/10)*0.95

			tiempo_resta = 10 - tiempo_desde_inicio_servida
			litros_resta = ferrosacumulados/precio
			litros_resta_int = int(ferrosacumulados/precio)
			
			
			display_servidos_lt((litros_servir - servidos_lt),10 - tiempo_desde_inicio_servida)
			writeLCD(4, litros_resta_int,tiempo_resta,0) ##FALTA ESTO
			#lcd_servidos_lt((litros_servir - servidos_lt),10 - tiempo_desde_inicio_servida)##LCD CAMBIAR##!!!!!
	...
	# deshabilita vavula y ozonizando
		...
		#lcd_agradece() ##LCD CAMBIAR##!!!!!
		writeLCD(5,0,0,0)
		sleep(0.5)
		
		#resetea el flujometro
		#ser_flw.write('aasdfasdf'.encode()) FLUJOMETRO
		bus.write_byte(0x06, 1)
		time.sleep(1)
