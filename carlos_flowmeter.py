...
def lcd_servidos_lt(servidos_lt,a):
    ser3.write(('  + ' + str(format(servidos_lt/1000, '.2f')) + ' litros! ').encode())  

def lcd_ahorradas_bot(ahorradas_bot,a):
    ser3.write(('  - ' + str(format(ahorradas_bot/1000, '.0f')) + ' botellas! ').encode())

def lcd_temperatura(temperatura,a):
        ser3.write(('Temperatura ' + str(format(temperatura))).encode())
...
while 1==1:
    ...
    if bytesToRead4 > 0:
            ...
            temperatura = string_igua
            print('lacadena recibidaes:' + temperatura)
            if int(loopcounter/int(2))%4 == 0:
                lcd_servidos_lt((servidos_lt),0)
                nada = 0
            if int(loopcounter/int(2))%4 == 1:
                lcd_ahorradas_bot(ahorradas_bot,0)
                nada = 0
            if int(loopcounter/int(2))%4 == 2:
                lcd_temperatura(temperatura,0)
                nada = 0
            if int(loopcounter/int(2))%4 == 3:
